...
export default function HomePage() {
  ...
  const executiveCockpit = (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KPICard title="Order Book Value" value={kpiVal('Order Book Value', '₫4.8T')} status="neutral" />
        <KPICard title="On-Time Delivery" value={kpiVal('On-Time Delivery', '91%')} status="warning" />
        <KPICard title="Active Orders" value={kpiVal('Active Orders', '1,247')} status="neutral" />
        <KPICard title="Customers" value={kpiVal('Customers', '87')} status="neutral" />
      </div>
  ...
  const domainTab1 = (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <KPICard title="Avg Lead Time" value={kpiVal('Avg Lead Time', '42 days')} />
        <KPICard title="Fill Rate" value={kpiVal('Fill Rate', '94%')} />
        <KPICard title="Cancellation Rate" value={kpiVal('Cancellation Rate', '2.4%')} />
      </div>
      <Chart
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Chart
          ...
          title="Payment Risk by Customer"
        />
        <ActionMemo
          persona={{ name: 'Pham Thi Huong', role: 'VP Commercial' }}
          context={{}}
          onGenerate={async () => {
            const memos = [
              {
                ...
                actions: ['Negotiate delivery extension for H&M spring order (fabric delay)', 'Accept Nike rush order with overtime premium', 'Follow up Zara payment (47 days overdue)'],
              },
              {
    ...
    <div className="h-[600px]">
      <AskAI
        ...
        sampleQuestions={[
          ...
          'Show seasonal demand pattern by customer',
          'What is the optimal capacity allocation for Q1 orders?',
        ]}
        mode="sql"
  ...
  const tabs = [
    ...
    { id: 'domain-2', label: 'Customer Risk', icon: '⚡', content: domainTab2 },
